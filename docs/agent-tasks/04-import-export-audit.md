...
**Статус: сделано.** Коммиты `be56074` (import + audit) и `5a47a31` (binary) на
ветке `feat/import-audit`. M2 milestone закрыт полностью.
...
|---|---|---|
| KeePass KDBX | `kdbx.go` | gokeepasslib/v3, вложенные группы, TOTP, аттачменты, пароль только с TTY |
| Bitwarden CSV | `bitwarden.go` | null URL → empty, normalizeTOTP |
...
| Chrome CSV | `chrome.go` | skip descriptive header row |
| Firefox CSV | `firefox.go` | URL → domain fallback через extractDomain |
| Enpass CSV | `enpass.go` | skip Recycle Bin, group fallback |
...
| `csvReadAll` | `csv.go` | BOM stripping, encoding detection, multiline fields |
...
| `ToSecret`/`StorePath`/`AttachmentStorePath` | `convert.go` | маппинг Entry → secret.Secret + store path |
...
2. **`iter.Seq2[Entry, error]` вместо `[]Entry`.** Потоковая обработка: KDBX с
   тысячами записей не грузится в память целиком.
...
4. **TOTP-секреты → `otpauth://`.** Raw base32 → `otpauth://totp/...`. Уже
   готовый URI — passthrough. Использует `pkg/otp`.
...
| `coverage_test.go` | 23 теста на coverage gaps (WriteEntries errors, partial entry, ToSecret, csvReadAll encoding/BOM, KDBX bad password, Registry no-match, extractDomain, Chrome negative header, Bitwarden null URL, normalizeTOTP, Enpass group, 1Password vault/category, Plan dedup) |
...
Покрытие: **89.5%**. Незакрытое: `runImport`/`runExport` CLI (требует mock store),
`fetchRange` http.NewRequestWithContext error branch.
...
Покрытие: **96.2%**. Незакрытое: `fetchRange` error branch (не injectable).
...
|---|---|---|
| `github.com/tobischo/gokeepasslib/v3` | v3 | KDBX parsing |
| `github.com/nbutton23/zxcvbn-go` | latest | password strength |
| `golang.org/x/text` | latest | encoding detection (CP1251, etc.) |
...
1. **60+ форматов.** 9 покрывает >90% real-world кейсов. Каждый новый формат —
   отдельный файл ~70 строк. Не сущесвенная разница в UX (auto-detect, --format).
